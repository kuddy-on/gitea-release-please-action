...
Runtime code lives in `src/`. `src/index.ts` is the Action entry point, `release-manager.ts` coordinates release pull requests and publishing, and `gitea-client.ts` wraps the Gitea REST API. Conventional Commit parsing, Markdown generation, markers, configuration, and shared types are kept in focused sibling modules.

Unit tests live in `test/` and mirror source modules, for example `test/conventional.test.ts`. `action.yml` defines the public interface, while committed runner-ready bundles are stored in `dist/`. Usage examples are in `examples/`; the Docker lifecycle test is `scripts/integration-test.sh`; repository CI is under `.github/workflows/`.
